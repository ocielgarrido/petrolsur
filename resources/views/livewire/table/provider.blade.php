<div>
    <x-data-table :data="$data" :model="$providers">
        <x-slot name="head">
            <tr>
        ...
        <x-slot name="body">
            @foreach ($providers as $provider)
                <tr x-data="window.__controller.dataTableController({{ $provider->id }})">
                   <td>{{ $provider->id }}</td>
                    ...
                    <td>{{ $provider->direccion}}</td>
                     <td class="whitespace-no-wrap row-action--icon">
                        <a role="button" href="/provider/edit/{{ $provider->id }}" class="mr-3"><i class="fa fa-16px fa-pen"></i></a>
