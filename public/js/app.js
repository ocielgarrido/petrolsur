...
function dataTableController(id) {
  return {
    ...
    recalculaItem: function recalculaItem() {
      var _this = this;
      ...
      Swal.fire({
        ...
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        confirmButtonText: 'Sí, Recaulcular !'
      }).then(function (result) {
        if (result.isConfirmed) {
          Livewire.emit('recalculaItem', _this.id);
        }
      });
    },
...
function dataTableMainController() {
  return {
    ...
    setCallback: function setCallback() {
      Livewire.on('recalculaResult', function (result) {
        if (result.status) {
          Swal.fire('Recalculado!', result.message, 'success');
        } else {
          Swal.fire('Error!', result.message, 'error');
        }
      });
